...
function getOptions(repo, token, page) {
    return {
      hostname: "api.github.com",
      port: 443,
      path: `/repos/${repo}/commits?${page ? 'page=' + page : ''}&per_page=100`,
      headers: {
...
const getMaxPages = (repo, token) => {
  return executeRequest(getOptions(repo, token), (res, resolve, reject) => {
    try {
...
const getCommits = (repo, token, page) => {
  return executeRequest(getOptions(repo, token, page), (res, resolve, reject) => {
